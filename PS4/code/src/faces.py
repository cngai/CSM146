...
def kMeans(points, k, init='random', plot=False, verbose=True) :
    """
    ...
    while True:
        ...
        if plot: plot_clusters(cur_cluster_set, 'kMeans Cluster Assignments - iteration {}, cheat initialization'.format(iters),
...
def kMedoids(points, k, init='random', plot=False, verbose = True) :
    """
    ...
    while True:
        ...
        cur_cluster_set = ClusterSet([Cluster(v) for _, v
                                      in compute_assignments_kMeans(
                                          cur_centroids, points).items()])
        iters+=1
        if plot: plot_clusters(cur_cluster_set, 'kMedoids Cluster Assignments - iteration {}, cheat initialization'.format(iters),
                               ClusterSet.medoids)
        # if verbose: print ("iters: {}".format(iters))
        if prev_clusts is not None and cur_cluster_set.equivalent(prev_clusts):
            # if verbose: print ("Done in {} iters".format(iters))
...
def main() :
    ...
    #              for i in range(num_eigenfaces_to_plot)]) #PART B
    for l in [1,10,50,100,500,1288]:
        ...
        #              for i in range(num_eigenfaces_to_plot)]) #PART C
    ...
    plt.plot(list(l_kmeans.keys()), list(l_kmeans.values()), 'b', label='k-means')
    plt.plot(list(l_kmed.keys()), list(l_kmed.values()), 'g', label='k-medoids')
